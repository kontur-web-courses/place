...
wss.on('connection', function connection(ws) {
    ws.on('message', (data) => {
        ...
        if (!(mess.type === 'setPoint'
            && mess.payload.x >= 0 && mess.payload.x < size
            && mess.payload.y >= 0 && mess.payload.y < size)) {
            return;
        }
        wss.clients.forEach((clt) => {
            if (clt.readyState === WebSocket.OPEN)
                clt.send(data);
        });
        place[mess.payload.x + size * mess.payload.y] = mess.payload.color;
    });
    ws.send(JSON.stringify({type: 'setPlace', payload: place}));
});
